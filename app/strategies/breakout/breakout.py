"""Range breakout strategy (long-only signals)."""
...
class BreakoutStrategy(TradingStrategy):
    """Trade clean breakouts above a consolidation range with volume confirmation."""
    ...
    def description(self) -> str:
        return "Buys when price breaks above a 20-bar range high on rising volume."
    ...
    def generate_features(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        frame["breakout_distance_pct"] = (
            frame["close"] - frame["range_high_20"]
        ) / frame["close"]
        return frame
    ...
    def generate_signals(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        for index in range(20, len(frame)):
            ...
            if any(value is None or pd.isna(value) for value in (range_high, width, volume_ratio)):
                continue
            if width < 0.06 and prev_close <= range_high < close and volume_ratio >= 1.15:
                signals[index] = 1
